...
func TranslationsPreInit() error {
	...
	return InitTranslationsWithDir("i18n-locales")
}
...
func GetTranslationsBySystemLocale() (i18n.TranslateFunc, error) {
	...
	if _, ok := locales[locale]; !ok {
		mlog.Error("Failed to load system translations for", mlog.String("locale", locale), mlog.String("attempting to fall back to default locale", model.DefaultLocale))
		locale = model.DefaultLocale
	}

	if locales[locale] == "" {
		return nil, fmt.Errorf("Failed to load system translations for '%v'", model.DefaultLocale)
	}
...
func GetTranslationsAndLocale(w http.ResponseWriter, r *http.Request) (i18n.TranslateFunc, string) {
	...
	translations := TfuncWithFallback(model.DefaultLocale)
	return translations, model.DefaultLocale
}
